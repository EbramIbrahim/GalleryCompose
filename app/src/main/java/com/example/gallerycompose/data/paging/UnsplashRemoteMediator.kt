...
class UnsplashRemoteMediator(
    ...
    private val unsplashImageDao = unsplashDatabase.imageDao()
    private val unsplashRemoteKeysDao = unsplashDatabase.remoteKeyDao()
    ...
    override suspend fun load(
        ...
        return try {
            val currentPage = when (loadType) {
                LoadType.REFRESH -> {
                    // REFRESH will be called the first time we make a request so if the next key = null
                    // so the value of the currentPage will = 1 first page
            ...
            unsplashDatabase.withTransaction {
                ...
                unsplashRemoteKeysDao.insertAllRemoteKeys(remoteKeys = keys)
    ...
    private suspend fun getRemoteKeyForFirstItem(
        ...
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { unsplashImage ->
                unsplashRemoteKeysDao.getRemoteKey(id = unsplashImage.id)
            }
    }
